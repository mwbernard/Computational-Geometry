...
void initialize_points_random() {
  ...
  int i; 
  for (i=0; i<n; i++) {
    points[i].x = (int)(.3*WINDOWSIZE)/2 + rand() % ((int)(.7*WINDOWSIZE));
    points[i].y =  (int)(.3*WINDOWSIZE)/2 + rand() % ((int)(.7*WINDOWSIZE));
  }
}
...
void initialize_points_square() {
    ...
    for (i=0; i<n; i++) {
        if (i % 4 == 0) {
            points[i].x = 100;
            points[i].y = rand() % ((WINDOWSIZE/5)*3 + 1) + (WINDOWSIZE/5);
        }
        else if (i % 5 == 1) {
            points[i].x = 400;
            points[i].y = rand() % ((WINDOWSIZE/5)*3 + 1) + (WINDOWSIZE/5);
            ...
            points[i].x = rand() % ((WINDOWSIZE/5)*3 + 1) + (WINDOWSIZE/5);
            points[i].y = 100;
        }
        else {
            points[i].x = rand() % ((WINDOWSIZE/5)*3 + 1) + (WINDOWSIZE/5);
...
// Function to draw diamond shape of points; does not work very well
/*
void initialize_points_diamond() {
    assert(points);
